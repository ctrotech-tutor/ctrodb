...
### Root Cause
File: `C:\Projects\project\web\survey\src\db\sync.ts`, lines 184-210
...
### Root Cause
File: `src/db/sync.ts`, lines 241-243
...
### Root Cause
File: `src/db/sync.ts`, lines 187 and 261-268
...
### Root cause
File: `src/db/sync.ts` — items are created with `status: "pending"`, updated to `"completed"` or `"failed"`, but never deleted.
...
### Reproduction
1. Sync starts, sets item status to `"syncing"` (line 152)
...
### Root Cause
File: `src/db/sync.ts`, lines 147-165
...
### Root Cause
File: `src/db/sync.ts`, lines 156-162
...
### Root Cause
File: `src/collection.ts`, line 46
...
### Root Cause
File: `src/adapter/idb.ts`, line 31
...
### Root Cause
File: `src/collection.ts`, in `create()` (lines 46-72):
...
## ISSUE 16: `Schema.validate()` Rejects Unknown Fields, Making Migrations Hard

### Severity: LOW

### Reproduction
1. Schema v1 has fields: `{ title: string, description: string }`
2. A record exists in IndexedDB with only `title` and `description`
3. Schema v2 adds `{ status: string, default: "draft" }`
4. `Schema.validate()` is called on the existing record (which doesn't have `status`)
5. `required` check passes (status is not required) — OK
6. Wait — actually the issue is on UPDATE: you `update(id, { status: "published" })`, which passes `{ status }` to validate. That's fine.
7. The real issue: if you try to `update(id, { description: "new desc" })`, Schema.validate() receives `{ description: "new desc" }`. It checks unknown fields (lines 141-146): any key in `data` that's not in `fieldDefs` is rejected. But `description` IS in `fieldDefs`, so that works.

Actually, the unknown field check works correctly here. The real migration issue is the FTS plugin store (Issue 1).

---

## ISSUE 17: No Built-in Sync Engine
...
## ISSUE 18: RelationsPlugin Monkey-Patches Collection via Proxy

### Severity: LOW

### Root Cause
File: `src/plugins/relations/index.ts`, lines 116-145
...
|---|-------|---------|----------|---------------|
| 1 | FTS plugin crashes on schema version bump | `src/adapter/idb.ts`, `src/plugins/fts/` | CRITICAL | Medium |
| 2 | Validation plugin rejects empty strings | `src/plugins/validation/index.ts` | HIGH | Small |
| 3 | `useQuery` returns bare array, no loading state | `src/react.ts` | HIGH | Medium |
| 4 | Sync engine only pulls `forms` collection | `src/db/sync.ts` (Formit) | HIGH | Small |
| 5 | No persistent sync cursor | `src/db/sync.ts` (Formit) | MEDIUM | Small |
| 6 | Race condition during pull drops local changes | `src/db/sync.ts` (Formit) | MEDIUM | Small |
| 7 | No conflict resolution (last-write-wins) | `src/db/sync.ts` (Formit) | MEDIUM | Medium |
| 8 | No sync_queue cleanup | `src/db/sync.ts` (Formit) | LOW | Small |
| 9 | Orphaned `syncing` items after crash | `src/db/sync.ts` (Formit) | LOW | Small |
| 10 | No exponential backoff for sync retries | `src/db/sync.ts` (Formit) | LOW | Small |
| 11 | `Partial<T>` on create lacks compile-time safety | `src/collection.ts` | LOW | Small |
| 12 | Mixed numeric/string IDs from autoIncrement | `src/adapter/idb.ts` | MEDIUM | Medium |
| 13 | `id_lookup` strategy never generated by planner | `src/query/planner.ts` | LOW | Small |
| 14 | `setDefaultDatabase` + `DatabaseProvider` dual API | `src/react.ts` | LOW | Small |
| 15 | Schema validation runs after plugin hooks | `src/collection.ts` | LOW | Small |
| 16 | No built-in sync engine | — (feature gap) | MEDIUM | Large |
| 17 | RelationsPlugin monkey-patches Collection | `src/plugins/relations/index.ts` | LOW | Medium |
